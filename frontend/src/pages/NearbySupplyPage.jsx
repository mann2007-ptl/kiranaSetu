                                ...
                                <Card>
                                    <div className="flex justify-between items-center mb-6">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <select
                                                ...
                                                className="px-3 py-2 rounded-lg border border-gray-200 bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 text-sm font-medium text-gray-900 cursor-pointer"
                                            >
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {filteredStores.map(store => (
                                            <div key={store.id} className="border border-gray-100 bg-white rounded-2xl p-5 hover:shadow-md transition-shadow">
                                                ...
                                                <div className="text-sm font-semibold text-gray-700 bg-gray-50 px-3 py-1.5 rounded-lg inline-flex items-center gap-1 mb-5 border border-gray-100">
                                    ...
                                    {filteredStores.length === 0 && (
                                        <div className="text-center py-12 px-4 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                                            ...
                                            <h3 className="text-sm font-bold text-gray-900">No stores found</h3>
                                            <p className="text-xs text-gray-500 max-w-sm mx-auto mt-1">Try expanding your search radius to see more partner stores.</p>
                                        </div>
                                    )}
                                </Card>
                            </div>
                                    ...
                                    <div className="space-y-4">
                                        {requests.filter(r => r.status === 'Pending').map(req => (
                                            <div key={req.id} className="p-4 bg-gray-50 rounded-xl border border-gray-100">
                                                <div className="flex justify-between items-start mb-2">
                                                    <span className="text-xs font-semibold text-gray-500 uppercase flex items-center gap-1.5"><MapPin size={12} /> {req.requestingStore}</span>
                                                    {getUrgencyBadge(req.urgency)}
                                                </div>
                                                <h3 className="font-bold text-gray-900 text-sm mb-1">{req.product}</h3>
                                                <p className="text-sm text-gray-700 font-medium mb-4">Qty Needed: <span className="text-indigo-600 font-bold">{req.quantity}</span></p>

                                                <div className="flex gap-2">
                                                    <Button variant="primary" className="flex-1 justify-center py-1.5 text-xs bg-emerald-600 border-none hover:bg-emerald-700 shadow-sm shadow-emerald-200" onClick={() => handleAccept(req.id)}>Accept</Button>
                                                    <Button variant="secondary" className="flex-1 justify-center py-1.5 text-xs hover:border-rose-200 hover:text-rose-600 hover:bg-rose-50" onClick={() => handleDecline(req.id)}>Decline</Button>
                                                </div>
                                            </div>
                                        ))}
                                        ...
                                        {requests.filter(r => r.status === 'Pending').length === 0 && (
                                            <div className="text-center py-8">
                                                <p className="text-sm font-semibold text-gray-500">All caught up! No pending requests.</p>
                                            </div>
                                        )}
                                    </div>
                                </Card>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        <Card className="relative w-full max-w-md z-10 p-0 overflow-hidden shadow-2xl">
                            <div className="px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                                <h2 className="text-lg font-bold text-gray-900">Request Product</h2>
                                <button onClick={() => setIsModalOpen(false)} className="text-gray-400 hover:text-rose-500 transition-colors p-1.5 rounded-md hover:bg-rose-50">
                                    <X size={20} />
                                </button>
                            </div>
                                    ...
                                    {() => (
                                        <Form className="space-y-5">
                                            <div>
                                                ...
                                                <Field name="product" type="text" className="w-full px-4 py-2.5 rounded-xl border border-gray-200 bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-sm font-medium transition-all" placeholder="e.g. Fortune Oil 1L" />
                                                <ErrorMessage name="product" component="div" className="text-rose-500 text-xs font-semibold mt-1" />
                                            </div>

                                            <div>
                                                <label className="block text-sm font-semibold text-gray-700 mb-1.5">Quantity Required</label>
                                                <Field name="quantity" type="number" className="w-full px-4 py-2.5 rounded-xl border border-gray-200 bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-sm font-medium transition-all" placeholder="Enter quantity" />
                                                <ErrorMessage name="quantity" component="div" className="text-rose-500 text-xs font-semibold mt-1" />
                                            </div>

                                            <div>
                                                <label className="block text-sm font-semibold text-gray-700 mb-1.5">Urgency Level</label>
                                                <Field as="select" name="urgency" className="w-full px-4 py-2.5 rounded-xl border border-gray-200 bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-sm font-medium transition-all cursor-pointer">
                                                ...
                                                <ErrorMessage name="urgency" component="div" className="text-rose-500 text-xs font-semibold mt-1" />
                                            </div>
