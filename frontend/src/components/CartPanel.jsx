            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar max-h-[340px]">
                {cartItems.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full min-h-[340px] p-5">
                        <div className="w-full h-full flex flex-col items-center justify-center border-2 border-dashed border-gray-200 rounded-2xl bg-gray-50/50">
                            <div className="w-16 h-16 bg-white rounded-2xl shadow-sm border border-gray-100 flex items-center justify-center mb-4 text-indigo-500">
                                <ShoppingCart size={28} strokeWidth={1.5} />
                            </div>
                            <p className="text-base font-bold text-gray-900">Your cart is empty</p>
                            <p className="text-sm text-gray-500 mt-1 px-4 text-center">
                ...
                <div className="border-t border-gray-100">
                    <div className="p-5 space-y-2.5">
                        <div className="flex justify-between text-sm">
                            ...
                            <span className="text-gray-700 font-semibold">₹{subtotal.toLocaleString('en-IN')}</span>
                        </div>
                        <div className="flex justify-between text-sm">
                            <span className="text-gray-500 font-medium">GST (18%)</span>
                            <span className="text-gray-700 font-semibold">₹{gst.toLocaleString('en-IN')}</span>
                        </div>
...
const CartItemRow = ({ item, onUpdateQuantity, onRemoveItem }) => {
    return (
        <div className="flex items-center gap-3 px-5 py-3.5 hover:bg-gray-50/60 transition-colors">
            {/* Image */}
            <div className="shrink-0 w-10 h-10 rounded-lg overflow-hidden bg-gray-100 border border-gray-200">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
                <button
                    ...
                    className="w-7 h-7 flex items-center justify-center rounded-lg bg-gray-100 text-gray-500 hover:bg-gray-200 transition-colors cursor-pointer"
                >
